...
void UI::LoginForm::Show(HINSTANCE hInstance, HWND parent){
	RECT rc;
	HWND hWndEnterWindow = CreateWindowW(ENTER_PASSWORD_CLASS_NAME, RtnResources::GetLabelLoginTitle().c_str(), WS_OVERLAPPEDWINDOW, 100, 100, 200, 200, NULL, NULL, hInstance, NULL);	
...
{	
	...
	rc.top+=20;
	UI::LoginForm::loginBox = CreateEdit(hwnd,cs->hInstance,0,rc,IDCE_SINGLELINE,_T(""));	
	rc.top+=40;	
	CreateLabel(hwnd,cs->hInstance,0,rc,IDCE_SINGLELINE,RtnResources::GetLabelPasswordText());
	rc.top+=20;
	UI::LoginForm::passwordBox = CreateEdit(hwnd,cs->hInstance,ES_PASSWORD,rc,IDCE_SINGLELINE, _T(""));
	rc.top+=40;
	rc.right-=73;
