...
// TODO not completed
public class MultiInstancesDiscriminatorDialog extends Dialog {
    ...
    private final List<String> subprocessVariableNames;

    public MultiInstancesDiscriminatorDialog(MultiSubprocess multiInstance) {
        super(PlatformUI.getWorkbench().getDisplay().getActiveShell());
        this.subprocessVariables = multiInstance.getVariablesList();
        this.definition = multiInstance.getProcessDefinition();
        subprocessVariableNames = getProcessVariablesNames(multiInstance.getSubProcessName());
        for (VariableMapping vm : multiInstance.getVariablesList()) {
            if (VariableMapping.USAGE_MULTIINSTANCE_VARS.equals(vm.getUsage())) {
    ...
    private void updateTabVariableSubProcessVariables() {
        ...
        tabVariableSubProcessVariables = subprocessVariableNames;
    ...
    public List<VariableMapping> getSubprocessVariables() {
        ...
        if (tabVariableProcessVariable.length() != 0) {
            ...
            vm.setUsage(VariableMapping.USAGE_MULTIINSTANCE_VARS);
