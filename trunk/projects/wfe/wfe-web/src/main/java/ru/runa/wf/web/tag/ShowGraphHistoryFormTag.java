...
public class ShowGraphHistoryFormTag extends ProcessBaseFormTag {
    ...
    protected void fillFormData(final TD formDataTD) throws JspException {
        ...
        try {
             ...
             * 
             * for (ProcessElementPresentation elementPresentation :
             * subprocesses) {
             * 
             * if (elementPresentation.isReadPermission()) {
             * 
             * int mlSize = 17; int maxItemsPerLine = 10; int additionalHeight =
             * 0; int mainDivSize = mlSize *
             * elementPresentation.getIds().size();
             * 
             * if (mainDivSize > (maxItemsPerLine * mlSize)) { additionalHeight
             * = (int) Math.ceil(mainDivSize / (maxItemsPerLine * mlSize)) *
             * mlSize; mainDivSize = maxItemsPerLine * mlSize; }
             * 
             ...
             * 
             * for (int i = 0; i < elementPresentation.raphCongetIds().size();
             * i++) { Long subprocessId = elementPresentation.getIds().get(i);
             ...
             * 
             * img.setUseMap("#processMap"); } } else { String toolTipId =
             * "log_" + logElements.indexOf(logElement) + "_tt"; StringBuffer
             * buf = new StringBuffer();buf.append(
