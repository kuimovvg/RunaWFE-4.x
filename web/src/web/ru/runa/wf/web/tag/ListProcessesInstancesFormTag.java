 ...
 * @author Gordienko_m
 * @jsp.tag name = "listProcessesInstancesForm" body-content = "JSP"
 */
public class ListProcessesInstancesFormTag extends BatchReturningTitledFormTag {
    ...
    protected void fillFormElement(TD tdFormElement) throws JspException {
        try {
            ...
            int instanceCount = executionService.getAllProcessInstanceStubsCount(getSubject(), batchPresentation);
            // we must call getProcessInstanceStubs before obtaining current page numper since it can be changed after getProcessInstanceStubs call
            List<ProcessInstanceStub> instances = executionService.getProcessInstanceStubs(getSubject(), batchPresentation);
            ...
            String[] prefixCellsHeaders = getGrouppingCells(batchPresentation, instances);
            HeaderBuilder headerBuilder = new SortingHeaderBuilder(batchPresentation, prefixCellsHeaders, new String[0], getReturnAction(),
                    pageContext);
            RowBuilder rowBuilder = new ReflectionRowBuilder(instances, batchPresentation, pageContext, ShowGraphModeHelper
                    .getManageProcessInstanceAction(), getReturnAction(), "id", builders);
    ...
    private String[] getGrouppingCells(BatchPresentation batchPresentation, List<ProcessInstanceStub> instances) {
        List<String> prefixCellsHeaders = new ArrayList<String>();
        int grouppingCells = GroupState.getMaxAdditionalCellsNum(batchPresentation, instances, new EnvImpl(batchPresentation));
    ...
    protected String getTitle() {
        return Messages.getMessage(Messages.TITLE_PROCESS_INSTANCES, pageContext);
    }
}
