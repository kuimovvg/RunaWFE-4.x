...
public class RedeployProcessDefinitionAction extends BaseDeployProcessDefinitionAction {
    ...
    protected void doAction(Subject subject, FileForm fileForm, List<String> processType, ActionMessages errors) throws FileNotFoundException,
        ...
        try {
            ProcessDefinition processDefinitionDescriptor = definitionService.getProcessDefinitionStub(subject, fileForm.getId());
            ...
            ProcessDefinition newProcessDefinitionStub = definitionService.getLatestProcessDefinitionStub(subject, processDefinitionDescriptor
                    .getName());
            definitionId = newProcessDefinitionStub.getNativeId();
    ...
    protected ActionForward getSuccessAction(ActionMapping mapping) {
        return Commons.forward(mapping.findForward(Resources.FORWARD_SUCCESS), IdForm.ID_INPUT_NAME, String.valueOf(definitionId));
    }

    @Override
    protected ActionForward getErrorForward(ActionMapping mapping) {
        if (definitionExists) {
            return Commons.forward(mapping.findForward(Resources.FORWARD_FAILURE), IdForm.ID_INPUT_NAME, String.valueOf(definitionId));
        }
        return mapping.findForward(ru.runa.wf.web.Resources.FORWARD_FAILURE_PROCESS_DEFINITION_DOES_NOT_EXIST);
    }
