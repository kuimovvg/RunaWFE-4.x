...
/**
 * @struts:action path="/updatePermissionOnRelation" name="updatePermissionsOnIdentifiableForm" validate="true" input =
 *                "/WEB-INF/af/manage_relation_members.jsp"
...
public class UpdatePermissionOnRelation extends UpdatePermissionOnIdentifiableAction {
    ...
    protected Identifiable getIdentifiable(Subject subject, Long identifiableId, ActionMessages errors) {
        try {
            return DelegateFactory.getInstance().getRelationService().getRelation(subject, identifiableId);
        } catch (Exception e) {
    ...
    protected ActionForward getErrorForward(Subject subject, ActionMapping mapping, Long identifiableId) {
        try {
            Relation relation = DelegateFactory.getInstance().getRelationService().getRelation(subject, identifiableId);
