...
public class UpdateExecutorDetailsForm extends IdForm {
    ...
    public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
        ...
        } else if (getNewName().length() > Resources.VALIDATOR_NAME_LENGTH) {
            errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(Messages.ERROR_WEB_CLIENT_VALIDATION));
        }

        if (getDescription() == null) {
            setDescription("");
        } else if (getDescription().length() > Resources.VALIDATOR_DESCRIPTION_LENGTH) {
            errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(Messages.ERROR_WEB_CLIENT_VALIDATION));
        }
        if (getFullName() == null) {
            setFullName("");
        } else if (getFullName().length() > Resources.VALIDATOR_FULL_NAME_LENGTH) {
            errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(Messages.ERROR_WEB_CLIENT_VALIDATION));
        }
        if (getEmail() == null) {
            setEmail("");
        } else if (getEmail().length() > Resources.VALIDATOR_EMAIL) {
            errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(Messages.ERROR_WEB_CLIENT_VALIDATION));
        }
        return errors;
    }
