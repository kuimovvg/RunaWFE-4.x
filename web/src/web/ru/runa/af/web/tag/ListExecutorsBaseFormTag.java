...
abstract public class ListExecutorsBaseFormTag extends UpdateExecutorBaseFormTag implements BatchedTag, ReturningTag {
    ...
    protected void fillFormData(TD tdFormElement) throws JspException {
        try {
            ...
            TDBuilder[] builders = getBuilders(new TDBuilder[] { new IdentifiableCheckboxTDBuilder(getExecutorsPermission()) }, batchPresentation,
                    new TDBuilder[] {});
            RowBuilder rowBuilder = new ReflectionRowBuilder(executors, batchPresentation, pageContext, Resources.ACTION_MAPPING_UPDATE_EXECUTOR,
     ...
     */
    protected abstract List<? extends Executor> getExecutors() throws AuthenticationException, AuthorizationException, JspException,
            BatchPresentationNotFoundException;

    /**
     * Load count of all executors may be shown in tag.
     * Used to setup pages before and after executor list.
     * @return Executors count. 
     */
    protected abstract int getExecutorsCount() throws AuthenticationException, AuthorizationException, JspException,
            BatchPresentationNotFoundException;
