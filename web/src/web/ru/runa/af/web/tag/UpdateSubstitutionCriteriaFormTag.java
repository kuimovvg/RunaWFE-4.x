        ...
        try {
            ...
            for (FunctionDef functionDef : functions) {
                ...
                for (ParamDef paramDef : functionDef.getParams()) {
                    ...
                    paramsDiv.append("<span>").append(createEditElement(paramDef.getRenderer(), getSubject(), pageContext, "", i, false)).append(
                            "</span>");
                    paramsDiv.append("</div>");
                }
                paramsDiv.append("</div>");
            ...
            if (criteriaType.length() > 0) {
                ...
                if (functionDef != null) {
                    for (int i = 0; i < functionDef.getParams().size(); i++) {
                        ...
                        table.addElement(createParameterTR(i, paramDef.getMessage(pageContext), createEditElement(paramDef.getRenderer(),
                                getSubject(), pageContext, value, i, isEnabled)));
                    }
                }
            }
            return table;
