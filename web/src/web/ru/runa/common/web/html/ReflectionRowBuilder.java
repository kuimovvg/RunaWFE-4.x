...
public class ReflectionRowBuilder implements RowBuilder {
    ...
    class EnvImpl extends EnvBaseImpl {
        ...
        public String getConfirmationMessage(Long pid) {
            if (basePartOfUrlToObject.equals(ru.runa.wf.web.Resources.ACTION_MAPPING_START_INSTANCE)
                    && ConfirmationPopupHelper.getInstance().isEnabled(ConfirmationPopupHelper.START_INSTANCE_PARAMETER)
                ...
                try {
                    if (!(definitionService.getStartInteraction(getSubject(), pid).hasFile() || definitionService.getOutputTransitionNames(getSubject(), pid, null)
                            .size() > 1)) {
                        String actionParameter = ConfirmationPopupHelper.START_INSTANCE_FORM_PARAMETER;
        ...
        public Object getTaskVariable(Object object, IdentifiableExtractor processIdExtractor, String variableName) throws AuthenticationException {
            ...
            if (cache == null) {
                ...
                List<Object> variables = executionService.getVariable(getSubject(), ids, variableName);
                for (int i = 0; i < items.size(); ++i) {
                    cache.put(ids.get(i), variables.get(i));
                }
            }
    ...
    public ReflectionRowBuilder(List<? extends Object> items, BatchPresentation batchPresentation, PageContext pageContext, String actionUrl, String returnAction,
    ...
    private ReflectionRowBuilder(List<? extends Object> items, BatchPresentation batchPresentation, PageContext pageContext, String actionUrl, String returnAction,
            TDBuilder[] builders) {
        this.items = items;
    ...
    private TR buildGroupHeader() {
        ...
        if (currentState.isVisible()) {
            groupingImage = new IMG(Commons.getUrl(Resources.GROUP_MINUS_IMAGE, pageContext, PortletUrl.Resource));
            groupingImage.setAlt(Resources.GROUP_MINUS_ALT);
        } else {
            groupingImage = new IMG(Commons.getUrl(Resources.GROUP_PLUS_IMAGE, pageContext, PortletUrl.Resource));
        ...
        String actionUrl = Commons.getActionUrl(ExpandCollapseGroupAction.ACTION_PATH, params, anchorId, pageContext, PortletUrl.Action);
    ...
    private class DefaultItemUrlStrategy implements ItemUrlStrategy {
        ...
        public String getUrl(String baseUrl, Object item) {
            try {
                ...
                return Commons.getActionUrl(baseUrl, IdForm.ID_INPUT_NAME, idValue, context, PortletUrl.Action);
            } catch (Exception e) {
                throw new InternalApplicationException(e);
            }
        }
