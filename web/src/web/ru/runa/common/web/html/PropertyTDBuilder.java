...
public class PropertyTDBuilder extends BaseTDBuilder {
    ...
    public TD build(Object object, Env env) throws JspException {
        ...
        if (authState == AUTH_STATE.ALWAYS_ENABLE || (authState == AUTH_STATE.ASK_WFE && isEnabled(object, env))) {
            ...
            if (object instanceof ProcessDefinition && env.getConfirmationMessage(((ProcessDefinition) object).getNativeId()) != null) {
                element.addAttribute("onclick", env.getConfirmationMessage(((ProcessDefinition) object).getNativeId()));
            }
        } else {
