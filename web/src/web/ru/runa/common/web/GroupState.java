...
public class GroupState {
    ...
    public static GroupState createStartState(List<? extends Object> items, BatchPresentation batchPresentation, TDBuilder[] builders, TDBuilder.Env env) {
    ...
    public String getGroupId() {
        ...
        for (int i = 0; i <= groupIndex; i++) {
            ...
            buf
                    .append(((TDBuilder) batchPresentation.getGrouppedFields()[i].getTDBuilder()).getSeparatedValues(items.get(itemIndex), env)[separatedValueNum[i]]);
        }
        return buf.toString();
    }
    ...
    private void incrementGroupIndex() {
        int sepCount = ((TDBuilder) batchPresentation.getGrouppedFields()[groupIndex].getTDBuilder()).getSeparatedValuesCount(items.get(itemIndex), env);
     ...
     * 
     * @param items to iterate
     * @param nextState to adjust
     * @return status, false means that no more items to iterate.
     */
     ...
     * 
     * @param batchPresentation to use
     * @param nextState to apply visibility
     */
    ...
    /**
     * Method must be invoked when current group state is: group header and not visible Then we iterate through items to find next item index for which next group state would be visible
     */
    ...
    private boolean isNextStateIsSubstateOfState(GroupState state, GroupState nextState) {
        ...
        for (int i = 0; i < state.getGroupIndex(); ++i) {
            ...
            if (!Arrays.deepEquals(tdBuilder.getSeparatedValues(items.get(state.itemIndex), env), tdBuilder.getSeparatedValues(
                    items.get(nextState.itemIndex), env))) {
                return false;
            }
        }
