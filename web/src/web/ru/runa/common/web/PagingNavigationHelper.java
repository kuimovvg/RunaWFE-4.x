...
public final class PagingNavigationHelper {
     ...
     * May be null if only instances count must be shown (without paging navigation).   
     */
     ...
     * @param pageContext Processing request page context.
     * @param batchPresentation {@linkplain BatchPresentation} used to load objects.
     * @param instanceCount All instances count (not only shown, but all available for {@linkplain BatchPresentation}).
     * @param returnAction Return action (current page). 
     */
    ...
    private TD createPagingNavigationTD(int pageCount) {
        ...
        if (currentPageNumber > 1) {
            addPageNovigationElement(pagingNovigationElementTD, currentPageNumber - 1, Messages.getMessage(Messages.LABEL_PAGING_PREV_PAGE,
                    pageContext));
            addDoubleNBSP(pagingNovigationElementTD);
        }
        ...
        if (currentPageNumber < pageCount) {
            addPageNovigationElement(pagingNovigationElementTD, currentPageNumber + 1, Messages.getMessage(Messages.LABEL_PAGING_NEXT_PAGE,
                    pageContext));
            addDoubleNBSP(pagingNovigationElementTD);
        }
        if (endIndex < pageCount) {
    ...
    private void addPageNovigationElement(TD listTd, int prevPageNumber, String textPresentation) {
        ...
        params.put(PagingForm.BATCH_PRESENTATION_ID, batchPresentation.getBatchPresentationId());
        params.put(ReturnActionForm.RETURN_ACTION, returnAction);
        params.put(PagingForm.VIEW_PAGE, String.valueOf(prevPageNumber));
        String actionUrl = Commons.getActionUrl(SetViewPageAction.ACTION_PATH, params, pageContext, PortletUrl.Action);
