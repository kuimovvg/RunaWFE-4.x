...
/**
 * Represnts tabs for managing different secured objects types.
...
public class TabHeaderTag extends TagSupport {
    ...
    static {
        ...
        FORWARDS.add(new MenuForward("manage_instances"));
        FORWARDS.add(new MenuForward("manage_executors"));
        FORWARDS.add(new MenuForward("manage_relations", RelationsGroupSecure.INSTANCE));
        FORWARDS.add(new MenuForward("configure_bot_station", BotStation.SECURED_INSTANCE));
        FORWARDS.add(new MenuForward("manage_system", ASystem.SYSTEM));
        //FORWARDS.add(new MenuForward("manage_settings", ASystem.SYSTEM));
    }
    ...
    public int doStartTag() throws JspException {
        ...
        if (isVertical()) {
            ...
            for (int i = 0; i < FORWARDS.size(); i++) {
                ...
                A a = getHref(Messages.getMessage(FORWARDS.get(i).getMenuName(), pageContext), FORWARDS.get(i).getMenuName());
                TD td = new TD();
                if (FORWARDS.get(i).equals(tabForwardName)) {
                    td.setClass(Resources.CLASS_TAB_CELL_SELECTED);
                }
                tr.addElement(td);
                td.addElement(a);
    ...
    private A getHref(String title, String forward) {
        String url = Commons.getForwardUrl(forward, pageContext, PortletUrl.Render);
