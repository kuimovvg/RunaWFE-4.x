...
!macro CreateRunSimulationBatchFile
  ...
  ${if} "$newSimulationDatabase" == "1"
    ...
    FileWrite $0 "  xcopy ..\server\default\demo-db $\"%APPDATA%\runawfe\jboss\data$\" /I /S /Y /R$\r$\n"
    FileWrite $0 ") else ($\r$\n"
    FileWrite $0 "  if not exist $\"%APPDATA%\runawfe\jboss\data$\" ($\r$\n"
    FileWrite $0 "    xcopy ..\server\default\demo-db $\"%APPDATA%\runawfe\jboss\data$\" /D /I /S /Y /R$\r$\n"
    FileWrite $0 "  )$\r$\n"
    FileWrite $0 ")$\r$\n"
  ${else}
    FileWrite $0 "if not exist $\"%APPDATA%\runawfe\jboss\data$\" ($\r$\n"
  ...
  ${if} "$simulationWebLinks" == "1"
    FileWrite $0 "del /S /Q ..\server\default\conf\simulation.properties$\r$\n"
  ${else}
    FileWrite $0 "echo skipLoginLinks=true>..\server\default\conf\simulation.properties$\r$\n"
  ${endif}
...
!macro installGPDSeq
  ...
  !insertmacro Runa_SetOutPath "$INSTDIR\gpd"
  File /r "${BuildRoot}\gpd-${AppVersion}\*"
  !insertmacro CreateRunGPDBatchFile
...
!macro installSimSeq
  ...
  !insertmacro createMenuShortcut "Stop Simulation.lnk" "$INSTDIR\Simulation\bin\nircmd.exe" "exec hide $\"$INSTDIR\Simulation\bin\runBatch.bat$\" shutdown.bat -S" "$INSTDIR\Simulation\bin" "$INSTDIR\Icons\SI_20x20_256.ico" "$(ShortcutDesc_StopSim)"
!macroend
...
!macro installBotstationSeq
  ...
  !insertmacro Runa_SetOutPath_INSIDE_CURRENTLOG "$INSTDIR\WFEServer\server\default"
  File /r "${BuildRoot}\wfe-botstation-config\server\default\*"
  Push "wfe_server"                            #text to be replaced
  Push $WFEServerAddress                       #replace with
  Push "$INSTDIR\WFEServer\server\default\conf\af_delegate.properties"   #file to replace in
  Call AdvReplaceInFile                        #call find and replace function
...
!macro installServerSeq
  ...
  !insertmacro installJbossSeq WFEServer
  !insertmacro Runa_SetOutPath_INSIDE_CURRENTLOG "$INSTDIR\WFEServer\server\default"
  File /r "${BuildRoot}\wfe-server-config\server\default\*"
...
!macro uninstallServerSeq
  SetShellVarContext all
  ExecShell open "$INSTDIR\WFEServer\bin\shutdown.bat" -S SW_HIDE
  Sleep 5000
