...
public class HibernateCompilerHQLBuider {
     ...
     */
    private final HibernateCompilerParameters parameters;
    ...
    /**
     * Map from HQL positional parameter name to parameter value.
     * All place holders must be put into this map (maybe with null values).
     * It will be used later to replace positional parameters in SQL query. 
     */
     ...
     * @param batchPresentation {@link BatchPresentation}, used to build HQL query.
     * @param parameters Parameters, used to build query.
     */
    public HibernateCompilerHQLBuider(BatchPresentation batchPresentation, HibernateCompilerParameters parameters) {
    ...
    /**
     * Check, if query has some filters on fields with inheritance.
     * This method must be called after build method call.
    ...
    /**
     * Check, if query has some sorting on fields with inheritance.
     * This method must be called after build method call.
    ...
    private void buildSelectClause() {
        ...
        if (parameters.getQueriedClass() != null) {
            ...
            query.append(batchPresentation.getClassPresentationClass().getName());
        }
    ...
    private List<String> addClassPresentationRestriction() {
        ...
        if (batchPresentation.getClassPresentationRestrictions() != null && !batchPresentation.getClassPresentationRestrictions().equals("")) {
            result.add(batchPresentation.getClassPresentationRestrictions());
        }
        return result;
    }
    ...
    private List<String> addJoinFieldRestrictions() {
        ...
        for (String alias : aliasMapping.getAliases()) {
            ...
            if (field.displayName.startsWith(ClassPresentation.removable_prefix)) {
                String propertyDBPath = field.displayName.substring(ClassPresentation.removable_prefix.length(), field.displayName.indexOf(':',
                        ClassPresentation.removable_prefix.length()));
                joinRestriction.append(" and (").append(alias).append(".").append(propertyDBPath).append("=:").append(
                        "removableUserValue" + field.fieldIdx).append(")");
                placeholders.put("removableUserValue" + field.fieldIdx, new QueryParameter("removableUserValue" + field.fieldIdx, field.displayName
                        .substring(field.displayName.lastIndexOf(':') + 1), Hibernate.STRING));
            }
    ...
    private List<String> addOwners() {
        ...
        Collection ownersIds = parameters.getOwners();
        if (ownersIds.isEmpty() || ownersIds.size() > 50 || ownersIds.iterator().next().getClass() != String.class) {
            owners.append("(").append(ClassPresentation.classNameSQL).append(".").append(parameters.getOwnerDBPath()).append(" in (:ownersIds) )");
            placeholders.put("ownersIds", null);
        } else {
            owners.append("(").append(ClassPresentation.classNameSQL).append(".").append(parameters.getOwnerDBPath()).append(" in (");
            //Dialect dialect = HibernateSessionFactory.getConfiguration().buildSettings().getDialect();
            String typeName = "VARCHAR";// TODO dialect.getCastTypeName(Hibernate.STRING.sqlType());
            for (int i = 1; i <= ownersIds.size(); ++i) {
                owners.append("CAST(").append(":ownersIds").append(i).append(" AS ").append(typeName).append(")").append(", ");
                placeholders.put("ownersIds" + i, null);
            }
            owners.replace(owners.length() - 2, owners.length(), "").append("))");
        }
        result.add(owners.toString());
        return result;
    }

    /**
     * Generates expressions to satisfy fields filtering restrictions.
     * This function doesn't generates filtering for fields with inheritance. It must be handled in SQL translation stage.
     * @return List of string, represents expressions.
     */
    private List<String> addFilters() {
        List<String> result = new LinkedList<String>();
        Map<Integer, FilterCriteria> fieldsToFilter = batchPresentation.getFilteredFieldsMap();
    ...
    private List<String> addSecureCheck() {
        ...
        String permissionQuery = "select pm.securedObject from ru.runa.af.dao.impl.PermissionMapping pm where pm.executor.id in (:securedOwnersIds) and pm.mask=:securedPermission";
        result.add("(instance.id in (select so.extId from ru.runa.af.SecuredObject so where so.type in (:securedTypes) and so.id in ("
            + permissionQuery + ")))");
    ...
    /**
     * Builds 'order by' clause of HQL query.
     * This function doesn't build sorting for fields with inheritance. It must be handled in SQL translation stage.
     */
    private void buildOrderClause() {
        ...
        for (int i = 0; i < sortedFields.length; i++) {
            ...
            query.append(needComma ? ", " : " ").append(sortedFields[i].dbSources[0].getValueDBPath(aliasMapping.getAlias(sortedFields[i]))).append(
                    fieldsToSortModes[i] ? " asc" : " desc");
            needComma = true;
        }
    }
}
