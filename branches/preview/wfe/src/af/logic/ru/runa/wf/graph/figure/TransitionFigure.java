...
public class TransitionFigure {
    ...
    public void draw(Graphics2D graphics, Color color) {
        ...
        if (bendPoint == null) {
            if ((figureFrom.getType() == NodeModel.FORK_JOIN) || DrawProperties.TIMEOUT_TRANSITION.equals(name)) {
                bendPoint = start;
            } else {
        ...
        if (actionsCount > 0) {
            ...
            boolean fromTimer = DrawProperties.TIMEOUT_TRANSITION.equals(name);
        ...
        if (exclusive) {
            ...
            if (DrawProperties.TIMEOUT_TRANSITION.equals(name)) {
        ...
        if (!DrawProperties.useEdgingOnly() && name != null && !name.startsWith("tr")) {
            String drawString = DrawProperties.TIMEOUT_TRANSITION.equals(name) ? timerInfo : name;
