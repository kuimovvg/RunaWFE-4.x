...
public class WfeScriptRunner {
    ...
    private Set<String> namedIdentitySet(Element element, boolean isGlobal) {
        ...
        for (int i = 0; i < nestedSet.getLength(); ++i) {
            if (!((Element) nestedSet.item(i)).getAttribute("type").equals(element.getAttribute("type"))) {
                new InternalApplicationException("Nested nameIdentitySet type is differs from parent type (For element " + name + " and type "
        ...
        if (isGlobal && name != null) {
            if (storage.put(name, result) != null) {
                new InternalApplicationException("Duplicate nameIdentitySet is found for name " + name + " and type " + element.getAttribute("type"));
            }
        }
        return result;
    }
    ...
    public void createActor(Element element) throws Exception, AuthenticationException, WeakPasswordException {
        ...
        actor = executorLogic.getActor(subject, name);
    ...
    public void deleteExecutor(Element element) throws ExecutorOutOfDateException, AuthorizationException, AuthenticationException {
        String name = element.getAttribute(NAME_ATTRIBUTE_NAME);
    ...
    public void addPermissionsOnProcessInstances(Element element) throws Exception {
        List<ProcessInstanceStub> list = getProcessInstanceStubs(element);
        if (list.size() == 0) {
            log.warn("Process instance " + element.getAttribute(NAME_ATTRIBUTE_NAME) + "doesn't exist");
        }
        for (ProcessInstanceStub pi : list) {
            addPermissionOnIdentifiable(element, pi);
        }
    }

    public void setPermissionsOnProcessInstances(Element element) throws Exception {
        List<ProcessInstanceStub> list = getProcessInstanceStubs(element);
        if (list.size() == 0) {
            log.warn("Process instance " + element.getAttribute(NAME_ATTRIBUTE_NAME) + "doesn't exist");
        }
    ...
    public void removePermissionsOnProcessInstances(Element element) throws Exception {
        List<ProcessInstanceStub> list = getProcessInstanceStubs(element);
        if (list.size() == 0) {
            log.warn("Process instance " + element.getAttribute(NAME_ATTRIBUTE_NAME) + "doesn't exist");
        }
    ...
    private List<ProcessInstanceStub> getProcessInstanceStubs(Element element) throws AuthenticationException {
        ...
        return processInstanceStub;
    }

    public void addPermissionsOnDefinition(Element element) throws Exception {
        for (ProcessDefinition definition : getProcessDefinitionStubs(element)) {
    ...
    private Set<ProcessDefinition> getProcessDefinitionStubs(Element element) throws Exception {
        Set<ProcessDefinition> result = new HashSet<ProcessDefinition>();
        for (String name : getNestedNamedIdentityNames(element, "ProcessDefinition", namedProcessDefinitionIdentities)) {
            result.add(definitionLogic.getLatestProcessDefinitionStub(subject, name));
        }
        return result;
    }

    private int processDeployed = 0;
    ...
    public void addPermissionsOnSystem(Element element) throws Exception {
        addPermissionOnIdentifiable(element, ASystem.SYSTEM);
    }

    public void setPermissionsOnSystem(Element element) throws Exception {
        setPermissionOnIdentifiable(element, ASystem.SYSTEM);
    }

    public void removePermissionsOnSystem(Element element) throws Exception {
        removePermissionOnIdentifiable(element, ASystem.SYSTEM);
    }
    ...
    public void removeAllPermissionsFromProcessInstances(Element element) throws Exception {
        List<ProcessInstanceStub> list = getProcessInstanceStubs(element);
        if (list.size() == 0) {
            log.warn("Process instance " + element.getAttribute(NAME_ATTRIBUTE_NAME) + "doesn't exist");
        }
    ...
    public void removeAllPermissionsFromSystem(Element element) throws Exception {
        removeAllPermissionOnIdentifiable(ASystem.SYSTEM);
    }
    ...
    private BatchPresentation getBatchFromProfile(Profile profile, String batchID, String batchName) throws BatchPresentationNotFoundException {
    ...
    public void replicateBatchPresentation(Map<BatchPresentation, ReplicationDescr> replicationDescr) throws Exception {
        ...
        List<Actor> allActors = executorLogic.getActors(subject,
                ru.runa.af.presentation.AFProfileStrategy.EXECUTOR_DEAFAULT_BATCH_PRESENTATOIN_FACTORY.getDefaultBatchPresentation());
        List<Long> actorsIds = Lists.newArrayListWithExpectedSize(allActors.size());
        for (Actor actor : allActors) {
            actorsIds.add(actor.getId());
        }
        List<Profile> profiles = profileLogic.getProfile(subject, actorsIds);
        // For all profiles
        for (Profile profile : profiles) {
            ...
            for (BatchPresentation replicateMe : replicationDescr.keySet()) {
                ...
                if (useTemplates
                        && !isBatchReplaceNeeded(
                                getBatchFromProfile(profile, replicateMe.getBatchPresentationId(), replicateMe.getBatchPresentationName()), templates)) {
    ...
    private Set<Actor> getActors(Element element) throws ExecutorOutOfDateException, AuthorizationException, AuthenticationException {
    ...
    public void changeSubstitutions(Element element) throws Exception {
        ...
        } catch (SubstitutionOutOfDateException e) {
    ...
    public void updateBotStation(Element element) throws Exception {
        ...
        botsLogic.update(subject, station);
    }
    ...
    public void addPermissionsOnBotStations(Element element) throws Exception {
        addPermissionOnIdentifiable(element, BotStation.SECURED_INSTANCE);
    }

    public void setPermissionsOnBotStations(Element element) throws Exception {
        setPermissionOnIdentifiable(element, BotStation.SECURED_INSTANCE);
    }

    public void removePermissionsOnBotStations(Element element) throws Exception {
        removePermissionOnIdentifiable(element, BotStation.SECURED_INSTANCE);
    }
        ...
        for (int i = 0; i < taskNodeList.getLength(); i++) {
            ...
            if (config == null || config.equals("")) {
                ...
                if (configContent == null) {
                    configContent = taskElement.getTextContent().trim(); // FIXME
                }
                if (configContent != null) {
                    configuration = configContent.getBytes("UTF-8");
                } else {
    ...
    public void removeAllConfigurationsFromBot(Element element) throws Exception {
        ...
        bot = botsLogic.getBot(subject, bot);
        if (bot == null) {
            log.warn("Bot " + botName + "doesn't exist");
        }
    ...
    public void removeOldProcessInstances(Element element) throws Exception {
        operationWithOldProcessInstances(element, 1);
    }
    ...
    public void archiveOldProcessInstances(Element element) throws Exception {
        operationWithOldProcessInstances(element, 2);
    }
    ...
    public void retrieveOldProcessInstances(Element element) throws Exception {
        operationWithOldProcessInstances(element, 3);
    }
    ...
    private void operationWithOldProcessInstances(Element element, int operation) throws Exception {
    ...
    public void relation(Element element) throws Exception {
        ...
        for (Relation group : relationLogic.getRelations(subject,
    ...
    public void stopProcessInstance(Element element) throws Exception {
        String id = element.getAttribute(ID_ATTRIBUTE_NAME);
        Long processInstanceId = Strings.isNullOrEmpty(id) ? null : Long.parseLong(id);
        executionLogic.cancelProcessInstance(subject, processInstanceId);
    }
}
