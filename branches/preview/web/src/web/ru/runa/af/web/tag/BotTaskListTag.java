...
public class BotTaskListTag extends TitledFormTag {
    ...
    protected void fillFormElement(TD tdFormElement) throws JspException {
        ...
        try {
            ...
            bot.setId(botID);
            AuthorizationService authorizationService = DelegateFactory.getInstance().getAuthorizationService();
            boolean disabled = !authorizationService.isAllowed(getSubject(), BotStationConfigurePermission.BOT_STATION_CONFIGURE,
                    BotStation.SECURED_INSTANCE);
    ...
    class BotTaskRowBuilder implements RowBuilder {
        ...
        private TD buildNameTD(BotTask task) {
            ...
            Input input = new Input(Input.TEXT,
                    BotTasksForm.BOT_TASK_INPUT_NAME_PREFIX + String.valueOf(task.getId()) + BotTasksForm.NAME_INPUT_NAME, task.getName());
        ...
        private TD buildHandlerTD(BotTask task) {
            ...
            select.setName(BotTasksForm.BOT_TASK_INPUT_NAME_PREFIX + String.valueOf(task.getId()) + BotTasksForm.HANDLER_INPUT_NAME);
    ...
    private TD buildConfigurationUploadTD(BotTask task) {
        ...
        Input fileUploadInput = new Input(Input.FILE, BotTasksForm.BOT_TASK_INPUT_NAME_PREFIX + String.valueOf(task.getId())
                + BotTasksForm.CONFIG_FILE_INPUT_NAME);
        ...
        if (task.getConfiguration() != null && task.getConfiguration().length > 0) {
            A link = new A(Commons.getActionUrl(BotTaskConfigurationFileDownLoader.DOWNLOAD_BOT_TASK_CONFIGURATION_ACTION_PATH, "id", task.getId(),
                    pageContext, PortletUrl.Action), Messages.getMessage(Messages.LABEL_BOT_TASK_CONFIG_DOWNLOAD, pageContext));
