...
public class HTMLFormConverter {
    ...
    public static byte[] changeUrls(PageContext pageContext, Long id, String htmlHref, byte[] originalBytes) throws SAXException,
            IOException {
    ...
    private static String getVarAsString(Map<String, Object> variables, String name) {
    ...
    public static byte[] setInputValues(PageContext pageContext, byte[] formBytes, Map<String, Object> variables,
        ...
        try {
            ...
            for (int i = 0; i < htmlTagElements.getLength(); i++) {
                ...
                String typeName = node.getAttribute(TYPE_ATTR);
                String inputName = node.getAttribute(NAME_ATTR);
                if (requiredVarNames != null && requiredVarNames.contains(inputName)) {
                    addRequiredClassAttribute(node);
                }
                String stringValue = getVarAsString(variables, inputName);
                if (stringValue == null) {
                    continue;
                }
                ...
                if (typeName == null || hasValue(STD_INPUT_NAMES, typeName, true)) {
                    ...
                    String checkBoxValue = node.getAttribute(VALUE_ATTR);
                    if (variables.get(inputName) instanceof String[]) {
                        List<String> selectedValues = Arrays.asList((String[]) variables.get(inputName));
    ...
    private static void addError(PageContext pageContext, Document document, Element node, String errorText, String inputName) {
        ...
        errorImg.setAttribute("src", Commons.getUrl("/images/error.gif", pageContext, PortletUrl.Resource));
