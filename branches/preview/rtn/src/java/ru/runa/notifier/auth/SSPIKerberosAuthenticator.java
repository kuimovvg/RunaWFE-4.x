...
/**
 * 
 * Комментарий The code has been written with help com.sun.jna.platform.win32.Secur32Test. Class get a HANDLE for outbound (client) credentials and
 * creates a client-side security context with help Security Support Provider Interface(SSPI)
 * 
...
public class SSPIKerberosAuthenticator implements Authenticator {
    ...
    public Subject authenticate() throws Exception {
        AuthenticationService authenticationServiceDelegate = DelegateFactory.getInstance().getAuthenticationService();
        ...
        try {
            ...
            int result = Secur32.INSTANCE.AcquireCredentialsHandle(null, "Kerberos", new NativeLong(Sspi.SECPKG_CRED_OUTBOUND), null, null,
            ...
            clientRc = Secur32.INSTANCE.InitializeSecurityContext(phClientCredential, null, LoginModuleResources.getServerPrincipal(),
                    new NativeLong(Sspi.ISC_REQ_CONNECTION), new NativeLong(0), new NativeLong(Sspi.SECURITY_NATIVE_DREP), null,
                    new NativeLong(0), phClientContext, pbClientToken, pfClientContextAttr, null);
            ...
            Subject subject = authenticationServiceDelegate.authenticate(pbClientToken.pBuffers[0].getBytes());
