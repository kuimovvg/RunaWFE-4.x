        ...
        try {
            ...
            for (JcrTask jcrTask : config.getTasks()) {
                if (JcrTask.GET_FILE.equals(jcrTask.getOperationName())) {
                    ...
                    outputVars.put(jcrTask.getVariableName(), fileVariable);
                }
                if (JcrTask.PUT_FILE.equals(jcrTask.getOperationName())) {
                    FileVariable fileVariable = (FileVariable) variables.get(jcrTask.getVariableName());
            ...
            executionService.completeTask(subject, taskStub.getId(), taskStub.getName(), taskStub.getTargetActor().getId(), outputVars);
            log.info("JCR TaskHandler completed, task " + taskStub);
        } catch (Exception e) {
