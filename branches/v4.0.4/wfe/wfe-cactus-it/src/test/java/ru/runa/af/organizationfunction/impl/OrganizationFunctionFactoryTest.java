...
public class OrganizationFunctionFactoryTest extends ServletTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        th.setPermissionsToAuthorizedPerformerOnDefinitionByName(Lists.newArrayList(DefinitionPermission.READ,
                DefinitionPermission.START_PROCESS, DefinitionPermission.UNDEPLOY_DEFINITION),
    ...
    public void testOrganizationFunction() throws Exception {
        List<WfTask> tasks = th.getExecutionService().getTasks(th.getErpOperatorUser(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(th.getAuthorizedPerformerUser(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(getBossActorUser(), th.getTaskBatchPresentation());
        ...
        th.getExecutionService().completeTask(getBossActorUser(), tasks.get(0).getId(), legalVariables, null);

        tasks = th.getExecutionService().getTasks(getBossActorUser(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(th.getErpOperatorUser(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 1, tasks.size());
        assertEquals("task name differs from expected", "updating erp asynchronously", tasks.get(0).getName());

        th.getExecutionService().completeTask(th.getErpOperatorUser(), tasks.get(0).getId(), legalVariables, null);

        tasks = th.getExecutionService().getTasks(th.getAuthorizedPerformerUser(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 1, tasks.size());
        assertEquals("task name differs from expected", "treating collegues on cake and pie", tasks.get(0).getName());

        th.getExecutionService().completeTask(th.getAuthorizedPerformerUser(), tasks.get(0).getId(), legalVariables, null);
