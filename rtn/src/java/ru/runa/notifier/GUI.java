...
public class GUI implements PropertyChangeListener, ViewChangeListener, LocationListener {
    ...
    private static InfoPathFormView infoPathFormView;
    private static final boolean isWindowsOS = "win32".equals(SWT.getPlatform()); // TODO find better way
    private static final boolean infoPathIntegration = ResourcesManager.isInfoPathIntergationEnabled();
    ...
    public void showBrowserView() {
        initBrowserView();
        openStartPage();
        if (isWindowsOS && infoPathIntegration) {
            minimazeControl(infoPathFormView);
        }
    ...
    public void showInfoPathFormView() {
        ...
        if (isWindowsOS && infoPathIntegration) {
            maximazeControl(infoPathFormView);
        }
        shell.layout(true, true);
    }
    ...
    private void initBrowserView() {
        if (browserView == null) {
            ...
            if (isWindowsOS && infoPathIntegration) {
                infoPathFormView = new InfoPathFormView(shell, SWT.FILL);
                infoPathFormView.setViewChangeListener(this);
                browserView.getBrowser().addLocationListener(infoPathFormView);
                infoPathFormView.setLayoutData(new GridData());
            }
