...
public class DelegateTaskServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	if(! WebResources.isTaskDelegationEnabled()) {
    		response.sendError(HttpServletResponse.SC_FORBIDDEN, "task.delegationEnabled");
    		return;
    	}
		...
		if(executors.size()==1) {
			...
			// temporary group
			String groupName = "delegateTaskGroup_" +
					user.getActor().getId().toString() + "_" +
					currentOwner.getId().toString() + "_" +
					taskId.toString(); 
