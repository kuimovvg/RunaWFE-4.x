...
public class UpgradeProcessToNextDefinitionVersionAction extends ActionBase {
    public static final String ACTION_PATH = "/upgradeProcessToNextDefinitionVersion";
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse responce) {
        ...
        try {
            Delegates.getExecutionService().upgradeProcessToNextDefinitionVersion(getLoggedUser(request), form.getId());
        } catch (Exception e) {
            addError(request, e);
            return Commons.forward(mapping.findForward(Resources.FORWARD_FAILURE), IdForm.ID_INPUT_NAME, form.getId());
        }
        addMessage(request, new ActionMessage(Messages.PROCESS_UPGRADED_TO_NEXT_DEFINITION_VERSION));
