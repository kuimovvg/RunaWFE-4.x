...
public class ShowGraphHistoryFormTag extends ProcessBaseFormTag {
    ...
    protected void fillFormData(final TD formDataTD) throws JspException {
        ...
        try {
             ...
             * 
             * if (mainDivSize > (maxItemsPerLine * mlSize)) { additionalHeight = (int) Math.ceil(mainDivSize / (maxItemsPerLine * mlSize)) * mlSize; mainDivSize = maxItemsPerLine
             * * mlSize; }
             * 
             * int[] ltCoords = new int[] { elementPresentation.getGraphConstraints()[2] - (mainDivSize / 2), elementPresentation.getGraphConstraints()[3] + (mlSize / 2) +
             * additionalHeight }; StringBuffer buf = new StringBuffer(); buf.append("<div class=\"multiInstanceContainer\" style=\" position: absolute; ");
             * buf.append("width: ").append(mainDivSize).append("px;"); buf.append("left: ").append(ltCoords[0] + 270).append("px;"); buf.append("top: ").append(ltCoords[1] +
             ...
             * buf.append("<a href=\"").append(getSubprocessHistoryInstanceUrl(subprocessId)).append("\" style>&nbsp;").append(i + 1).append("&nbsp;</a>"); buf.append("</div>");
             * 
             ...
             * 
             * img.setUseMap("#processMap"); } } else { String toolTipId = "log_" + logElements.indexOf(logElement) + "_tt"; StringBuffer buf = new StringBuffer();buf.append(
             ...
             * buf.append("width: ").append(logElement.getGraphCoordinates()[2]).append("px;"); buf.append("height: ").append(logElement.getGraphCoordinates()[3]).append("px;");
             * buf.append("left: ").append(logElement.getGraphCoordinates()[0] + 270).append("px;"); buf.append("top: ").append(logElement.getGraphCoordinates()[1] +
             * 150).append("px;\" onmouseover=\"showTip(event, '").append(toolTipId).append ("')\" onmouseout=\" hideTip('").append(toolTipId).append("')\" >");
             * 
             * buf.append("</div>");
             * 
             * buf.append("<span id=\""); buf.append(toolTipId); buf.append("\" class=\"field-hint\" style=\"display: none;\">"); buf.append(logElement.getLog());
             * buf.append("</span>");
             * 
