...
public abstract class AlfExecuteWebScriptHandler extends AlfHandler {
    ...
    protected void executeAction(AlfConnection alfConnection, AlfHandlerData alfHandlerData) throws Exception {
        byte[] response = getResponse(alfConnection, alfHandlerData);
        log.debug(new String(response, Charsets.UTF_8.name()));
        handleResponse(alfHandlerData, response);
    }

    protected byte[] getResponse(AlfConnection alfConnection, AlfHandlerData alfHandlerData) throws Exception {
