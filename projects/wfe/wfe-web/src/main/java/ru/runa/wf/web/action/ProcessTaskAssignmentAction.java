...
public class ProcessTaskAssignmentAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isOneTaskProcessing) {
            ...
            taskPreviousOwners.add(getExecutor(subject, request.getParameter(ru.runa.common.WebResources.HIDDEN_TASK_PREVIOUS_OWNER_ID)));
        } else {
            for (String strId : idsForm.getStrIds()) {
                String[] ids = strId.split(":", -1);
                taskIds.add(Long.parseLong(ids[0]));
                taskPreviousOwners.add(getExecutor(subject, ids[1]));
            }
        }
        try {
            ...
            for (int i = 0; i < taskIds.size(); i++) {
                try {
                    ...
                    executionService.assignTask(subject, taskId, previousExecutor, actor);
    ...
    private Executor getExecutor(Subject subject, String idString) {
