		...
		tableViewer.setLabelProvider(new ITableLabelProvider(){
			public Image getColumnImage(Object element, int columnIndex) {
				if(element instanceof File){
					return HTMLPlugin.getImage(HTMLPlugin.ICON_JAR_EXT);
				} else if(element instanceof IFile){
					return HTMLPlugin.getImage(HTMLPlugin.ICON_JAR);
				}
				return null;
			}
		...
		add.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent evt){
				...
				dialog.setValidator(new ISelectionStatusValidator(){
					private IStatus okStatus = new Status(Status.OK, "EditorsPlugin", Status.OK, "", null);
					private IStatus ngStatus = new Status(Status.ERROR, "EditorsPlugin", Status.ERROR, "", null);
