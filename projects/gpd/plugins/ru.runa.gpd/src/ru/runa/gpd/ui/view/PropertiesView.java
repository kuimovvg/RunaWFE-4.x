...
public class PropertiesView extends ViewPart implements ISelectionListener,
	...
	private void handleSelect(TreeItem item) {
		for (TreeItem treeItem : tree.getItems()) {
			CellEditor cellEditor = (CellEditor) treeItem
					.getData(CELL_EDITOR_KEY);
			if (cellEditor != null && !cellEditor.getControl().isDisposed()
					&& !cellEditor.getControl().isVisible()) {
	...
	public Object getAdapter(Class adapter) {
		if (adapter == IHelpToticProvider.class && sourcePart != null) {
			return new HelpTopicProviderProxy(originalSelection,
					(IHelpToticProvider) sourcePart.getAdapter(adapter));
		}
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		...
		if (part instanceof GraphitiProcessEditor) {
			newSource = ((GraphitiProcessEditor) part)
		...
		if (selection instanceof IStructuredSelection) {
			...
			if (!structuredSelection.isEmpty()) {
				...
				if (sel instanceof IPropertySource) {
					...
					source = (IPropertySource) ((IAdaptable) sel)
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		for (TreeItem item : tree.getItems()) {
			IPropertyDescriptor descriptor = (IPropertyDescriptor) item
					.getData(PROPERTY_DESCRIPTOR_KEY);
			if (descriptor != null
					&& descriptor.getId().equals(evt.getPropertyName())) {
				CellEditorListener cellEditorListener = (CellEditorListener) item
						.getData(CELL_EDITOR_LISTENER_KEY);
	...
	private void updateChildren() {
		...
		for (int i = 0; i < descriptors.length; i++) {
			...
			if (source instanceof IRequiredPropertiesSource) {
				isPropertyRequired = ((IRequiredPropertiesSource) source)
						.isPropertyRequired(descriptor.getId());
			}
			...
			if (cellEditor != null) {
				...
				cellEditorListener = new CellEditorListener(item, descriptor,
						cellEditor);
				...
				treeEditor.setEditor(cellEditor.getControl(), item,
						columnToEdit);
    ...
    public class CellEditorListener implements ICellEditorListener {
		...
		public CellEditorListener(TreeItem item,
				IPropertyDescriptor descriptor, CellEditor cellEditor) {
			this.item = item;
	...
	private static class HelpTopicProviderProxy implements IHelpToticProvider {
		private ISelection selection;
		private IHelpToticProvider provider;

		public HelpTopicProviderProxy(ISelection selection,
				IHelpToticProvider provider) {
