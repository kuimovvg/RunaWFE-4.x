...
public class TreeCombo extends Composite {
	...
	Font font;
	List<TreeComboItem> items = new ArrayList<TreeComboItem>();
	List<TreeComboColumn> columns = new ArrayList<TreeComboColumn>();
	List<TreeListener> treeListeners = new ArrayList<TreeListener>();

	private TreeListener hookListener = new TreeListener() {

		public void treeCollapsed(TreeEvent e) {
			e.item = (Widget) e.item.getData(TreeComboItem.DATA_ID);
			e.widget = TreeCombo.this;
	...
	public TreeCombo(Composite parent, int style) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (TreeCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == TreeCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
		...
		int[] textEvents = { SWT.DefaultSelection, SWT.KeyDown, SWT.KeyUp,
				...
				SWT.MouseDoubleClick, SWT.MouseWheel, SWT.Traverse,
				SWT.FocusIn, SWT.Verify };
		for (int i = 0; i < textEvents.length; i++)
			text.addListener(textEvents[i], listener);

		int[] arrowEvents = { SWT.MouseDown, SWT.MouseUp, SWT.Selection,
				SWT.FocusIn };
	...
	void dropDown(boolean drop) {
		...
		if (getShell() != popup.getParent()) {
			...
			TreeComboItem selectionIndex = null;

			if (s.length > 0) {
				selectionIndex = (TreeComboItem) s[0]
						.getData(TreeComboItem.DATA_ID);
			}
		...
		// if (y + height > displayRect.y + displayRect.height) y = parentRect.y
		// - height;
		// if (x + width > displayRect.x + displayRect.width) x = displayRect.x
		// + displayRect.width - listRect.width;
	...
	void textEvent(Event event) {
		...
		// TreeComboItem oldItem = internalGetSelection();
	...
	void listEvent(Event event) {
		...
		case SWT.Dispose:
			if (getShell() != popup.getParent()) {
				TreeComboItem selectionIndex = internalGetSelection();
				popup = null;
				tree = null;
				createPopup(items, selectionIndex);
			}
	...
	void createPopup(Collection<TreeComboItem> items, TreeComboItem selectedItem) {
		...
		int[] listEvents = { SWT.MouseUp, SWT.Selection, SWT.Traverse,
				SWT.KeyDown, SWT.KeyUp, SWT.FocusIn, SWT.Dispose, SWT.Collapse,
				SWT.Expand };
		for (int i = 0; i < listEvents.length; i++)
			tree.addListener(listEvents[i], listener);

		for (TreeComboColumn c : columns) {
			TreeColumn col = new TreeColumn(tree, SWT.NONE);
			c.setRealTreeColumn(col);
		}

		if (items != null) {
			createTreeItems(items.toArray(new TreeComboItem[0]));
		}
	...
	private void createTreeItems(TreeComboItem[] items) {
	...
	public TreeComboItem[] getSelection() {
		checkWidget();

		TreeItem[] items = tree.getSelection();

		if (items.length > 0) {
			return new TreeComboItem[] { (TreeComboItem) items[0]
					.getData(TreeComboItem.DATA_ID) };
		}

		return new TreeComboItem[0];
	}

	private TreeComboItem internalGetSelection() {
		TreeComboItem[] is = getSelection();
		if (is.length != 0) {
			return is[0];
		}
	...
	public void select(TreeComboItem item) {
		checkWidget();
		if (item == null) {
			tree.deselectAll();
			text.setFont(text.getFont());
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		for (int i = 0; i < items.length; i++) {
			textWidth = Math.max(gc.stringExtent(items[i].getText()).x,
					textWidth);
		}
		gc.dispose();
	...
	public TreeComboItem getItem(int index) {
	...
	public TreeComboColumn getColumn(int columnIndex) {
		return columns.get(columnIndex);
	}

	public void setSelection(TreeComboItem[] newItems) {
	...
	public TreeComboItem[] getItems() {
		return items.toArray(new TreeComboItem[0]);
	}

	public TreeComboItem getItem(Point p) {
		TreeItem item = tree.getItem(p);

		if (item != null) {
			item.getData(TreeComboItem.DATA_ID);
		}
	...
	public void showItem(TreeComboItem item) {
	...
	public void addSelectionListener(SelectionListener listener) {
		...
		tree.addListener(SWT.Selection, typedListener);
		tree.addListener(SWT.DefaultSelection, typedListener);
	}
	...
	public void selectByFullPath(String[] itemFullPath) {
		...
		TreeComboItem rootItem = findRoot(itemFullPath[0]);
		
		if (itemFullPath.length == 1) {
			select(rootItem);
		}
		else {
			select(findChildItem(rootItem, itemFullPath, 1));
		}
	}
	
	private TreeComboItem findRoot(String rootName) {
		if (rootName != null) {
			for (TreeComboItem treeComboItem : items) {
				if (rootName.equals(treeComboItem.getText()) && treeComboItem.getParentItem() == null) {
					return treeComboItem;
				}
			}
		}			
	...
	private TreeComboItem findChildItem(TreeComboItem root, String[] fullPath, int startFrom) {		
		for (TreeComboItem treeComboItem : root.getItems()) {
			if (fullPath[startFrom].equals(treeComboItem.getText())) {
				if (startFrom == fullPath.length - 1) {
					return treeComboItem;
				}
				else {
					return findChildItem(treeComboItem, fullPath, startFrom + 1);
				}
			}
		}
		return null;	
	...
	public TreeComboItem getSelectionIndex() {
                TreeItem[] s = tree.getSelection();
		TreeComboItem selectionIndex = null;
	...
	public void add(TreeComboItem treeComboItem, int i) {
		TreeComboItem[] newItem = new TreeComboItem[1];
		createTreeItems(newItem);		
	}
