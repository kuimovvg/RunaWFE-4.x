...
# TEST PRIORITY: 0
# SPEC ID: -
# OVERVIEW: Данный shell-скрипт запускает тест-сьют runaex_Test_suite_CRUD.html содержащий тест-кейсы для стандартной проверки работоспособности интерфейса.
...
if [ x"$selDebug" != x"off" ]
then
	java -jar $selServer -debug -htmlSuite $selBrowserString $selStartURL $selSuiteFile $selResultFile -port $selPort -timeout $selTimeout -userExtensions $selExtensions -firefoxProfileTemplate $ffProfile
else
