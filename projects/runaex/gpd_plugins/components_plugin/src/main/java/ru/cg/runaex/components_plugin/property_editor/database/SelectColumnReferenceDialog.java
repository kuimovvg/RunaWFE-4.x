...
public class SelectColumnReferenceDialog extends Dialog {
    private Tree tree;
    private ColumnReference selectedColumnReference;
    ...
    public SelectColumnReferenceDialog(IProject currentProject) {
        super(Display.getCurrent().getActiveShell());
        localization = DescriptorLocalizationFactory.getSelectColumnReferenceDialogLocalization();
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        viewer.setContentProvider(new DBResourcesContentProvider(area.getShell(), currentProject));
        ...
        viewer.addSelectionChangedListener(new ISelectionChangedListener() {
            ...
            public void selectionChanged(final SelectionChangedEvent event) {
                if (!event.getSelection().isEmpty() && ((TreeSelection) event.getSelection()).size() == 1
                        && !(((TreeSelection) event.getSelection()).getFirstElement().getClass().getName().equals("org.apache.ddlutils.model.Column"))) {
    ...
    private void onItemSelected() {
        ...
        if (selection.length > 0) {
            ...
            String column = treeItem.getText();
            treeItem = treeItem.getParentItem();
            String table = treeItem.getText();
            treeItem = treeItem.getParentItem();
            String schema = treeItem.getText();
            selectedColumnReference = new ColumnReference(schema, table, column, 3);
        } else {
            selectedColumnReference = null;
        }
    }

    public ColumnReference getSelectedColumnReference() {
        return selectedColumnReference;
    }
