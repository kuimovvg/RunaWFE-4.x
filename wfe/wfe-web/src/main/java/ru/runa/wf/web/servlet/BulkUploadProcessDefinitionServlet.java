...
public class BulkUploadProcessDefinitionServlet extends HttpServlet {
	 ...
	 public static final String UPLOADED_PAR_FILES = "UploadedParFiles";
	    ...
	    public Map<String, UploadedFile> getUploadedParFilesMap(HttpServletRequest request) {
	        Map<String, UploadedFile> map = (Map<String, UploadedFile>) request.getSession().getAttribute(UPLOADED_PAR_FILES);
	        if (map == null) {
	            map = Maps.newHashMap();
	            request.getSession().setAttribute(UPLOADED_PAR_FILES, map);
	        }
	        return map;
	    }
}
