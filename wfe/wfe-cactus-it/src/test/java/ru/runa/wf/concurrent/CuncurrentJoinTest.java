...
public class CuncurrentJoinTest extends ServletTestCase {
    ...
    public void testConcurrentTaskExecution() throws Exception {
         ...
         * executionService.getTasks(helper.getAuthorizedPerformerUser(), BatchPresentationFactory.TASKS.createDefault()); assertEquals(2,
         ...
         * executionService.completeTask(helper.getAuthorizedPerformerUser(), task.getId(), new HashMap<String, Object>(), null); } catch (Exception
         * e) { } semaphore.release(); } }).start(); WfTask task = tasks.get(0).getName().contains("1") ? tasks.get(1) : tasks.get(0);
         * executionService.completeTask(helper.getAuthorizedPerformerUser(), task.getId(), new HashMap<String, Object>(), null); semaphore.acquire();
         * List<WfTask> newTasks = executionService.getTasks(helper.getAuthorizedPerformerUser(), BatchPresentationFactory.TASKS.createDefault());
