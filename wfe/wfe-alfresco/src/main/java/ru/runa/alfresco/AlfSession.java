...
public class AlfSession implements AlfConn {
	...
	public String createFolder(String rootFolderUUID, String folderName) throws InternalApplicationException {
		try {
			...
			CMLCreate createDoc = new CMLCreate("ref1", docParent, null, ContentModel.ASSOC_CONTAINS.toString(), folderName, ContentModel.TYPE_FOLDER.toString(), props);
			CML cml = new CML();
	...
	public void addAssociation(AlfObject source, AlfObject target, String associationName) throws InternalApplicationException {
		try {
			AlfSessionWrapper.sessionStart();
			CMLCreateAssociation createAssociation = new CMLCreateAssociation(source.getPredicate(), null, target.getPredicate(), null, associationName);
			CML cml = new CML();
	...
	public void addChildAssociation(AlfObject source, AlfObject target, String associationName) throws InternalApplicationException {
		try {
			AlfSessionWrapper.sessionStart();
			ParentReference parentReference = new ParentReference(source.getReference().getStore(), source.getReference().getUuid(), null, associationName, associationName);
		...
		try {
			...
			for (AlfSerializerDesc desc : assocToDelete.keySet()) {
				...
				for (Object ref : refs) {
					...
					if (desc.getAssoc().child()) {
						...
						CMLRemoveAssociation removeAssociation = new CMLRemoveAssociation(object.getPredicate(), null, where, null, desc.getPropertyNameWithNamespace());
			...
			for (AlfSerializerDesc desc : assocToCreate.keySet()) {
				...
				for (Object ref : refs) {
					...
					if (desc.getAssoc().child()) {
						ParentReference parentReference = new ParentReference(object.getReference().getStore(), object.getReference().getUuid(), null,
								desc.getPropertyNameWithNamespace(), target.getObjectName());
						CMLAddChild addChild = new CMLAddChild(parentReference, null, desc.getPropertyNameWithNamespace(), target.getObjectName(), predicate, null);
						addChilds.add(addChild);
					} else {
						CMLCreateAssociation createAssociation = new CMLCreateAssociation(object.getPredicate(), null, predicate, null, desc.getPropertyNameWithNamespace());
	...
	public <T extends AlfObject> T buildObject(String typeName, Reference reference, NamedValue[] props, String[] aspects) throws InternalApplicationException {
		try {
	...
	public void setContent(AlfObject object, byte[] content, String mimetype) throws InternalApplicationException {
		try {
			...
			WebServiceFactory.getContentService().write(object.getReference(), Constants.PROP_CONTENT, content, new ContentFormat(mimetype, Charsets.UTF_8.name()));
		} catch (Exception e) {
			throw propagate(e);
		} finally {
			AlfSessionWrapper.sessionEnd();
