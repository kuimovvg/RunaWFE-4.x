...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:src="http://schemas.microsoft.com/wix/2003/01/wi" xmlns="http://schemas.microsoft.com/wix/2003/01/wi">
  ...
  <xsl:template match="src:Directory[name(..)='DirectoryRef']">
    ...
    <xsl:element name="ComponentGroup">
      <xsl:choose>
        <xsl:when test="starts-with(@LongName, 'rtn')">
          <xsl:attribute name="Id">RTN_ComponentsGroup</xsl:attribute>
        </xsl:when>
        <xsl:when test="starts-with(@LongName, 'gpd')">
          <xsl:attribute name="Id">GPD_ComponentsGroup</xsl:attribute>
        </xsl:when>
        <xsl:when test="starts-with(@LongName, 'wfe')">
          <xsl:attribute name="Id">JBOSS_ComponentsGroup</xsl:attribute>
        </xsl:when>
        <xsl:when test="starts-with(@LongName, 'Doc')">
          <xsl:attribute name="Id">DOC_ComponentsGroup</xsl:attribute>
        </xsl:when>
      </xsl:choose>
      <xsl:apply-templates select="@*|node()|text()"/>
    <xsl:text>
