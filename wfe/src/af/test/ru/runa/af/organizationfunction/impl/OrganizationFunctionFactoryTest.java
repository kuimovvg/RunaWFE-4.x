...
public class OrganizationFunctionFactoryTest extends ServletTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        th.setPermissionsToAuthorizedPerformerOnDefinitionByName(Lists.newArrayList(ProcessDefinitionPermission.READ,
                ProcessDefinitionPermission.START_PROCESS, ProcessDefinitionPermission.UNDEPLOY_DEFINITION),
                WfServiceTestHelper.SWIMLANE_PROCESS_NAME);

        th.getExecutionService().startProcessInstance(th.getAuthorizedPerformerSubject(), WfServiceTestHelper.SWIMLANE_PROCESS_NAME);
    ...
    private Subject getBossActorSubject() {
        return th.getHrOperatorSubject();
    }
    ...
    public void testOrganizationFunction() throws Exception {
        List<TaskStub> tasks = th.getExecutionService().getTasks(th.getErpOperatorSubject(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(th.getAuthorizedPerformerSubject(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(getBossActorSubject(), th.getTaskBatchPresentation());
        ...
        th.getExecutionService().completeTask(getBossActorSubject(), tasks.get(0).getId(), tasks.get(0).getName(), tasks.get(0).getTargetActor().getId(),
                legalVariables);

        tasks = th.getExecutionService().getTasks(getBossActorSubject(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 0, tasks.size());

        tasks = th.getExecutionService().getTasks(th.getErpOperatorSubject(), th.getTaskBatchPresentation());
        assertEquals("tasks count differs from expected", 1, tasks.size());
        assertEquals("task name differs from expected", "updating erp asynchronously", tasks.get(0).getName());

        th.getExecutionService().completeTask(th.getErpOperatorSubject(), tasks.get(0).getId(), tasks.get(0).getName(),
                tasks.get(0).getTargetActor().getId(), legalVariables);
