...
public class CalendarInterval implements Comparable<CalendarInterval> {
    ...
    public static void main(String[] args){
        CalendarInterval interval = new CalendarInterval(CalendarUtil.convertToCalendar("2012-07-01",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-07-07",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
        
        List<CalendarInterval> list = new ArrayList<CalendarInterval>();
        
        CalendarInterval interval1 = new CalendarInterval(CalendarUtil.convertToCalendar("2012-06-25",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-07-02",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
        list.add(interval1);
        
        CalendarInterval interval2 = new CalendarInterval(CalendarUtil.convertToCalendar("2012-07-04",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-07-05",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
        list.add(interval2);
        
        CalendarInterval interval3 = new CalendarInterval(CalendarUtil.convertToCalendar("2012-07-06",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-07-08",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
        list.add(interval3);
        
//        CalendarInterval interval4 = new CalendarInterval(CalendarUtil.convertToCalendar("2012-06-26",
//                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-07-04",
//                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
//        list.add(interval4);
        
        CalendarInterval interval5 = new CalendarInterval(CalendarUtil.convertToCalendar("2012-06-26",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT), CalendarUtil.convertToCalendar("2012-06-28",
                CalendarUtil.ALF_DATE_WITHOUT_TIME_FORMAT));
        list.add(interval5);
        
        
        CalendarUtil.setLastSecondTimeCalendar(interval2.getTo());
        CalendarUtil.setZeroTimeCalendar(interval3.getFrom());
        
        System.out.println("interval = "+interval.toString());
        int i = 1;
        for (CalendarInterval calendarInterval: list){
            System.out.println("interval "+ (i++) +" = "+calendarInterval.toString());
        }
        
        List<CalendarInterval> croppedList = interval.cropToFitInInterval(list);
        i = 1;
        System.out.println("Cropped:");
        for (CalendarInterval calendarInterval: croppedList){
            System.out.println("interval "+ (i++) +" = "+calendarInterval.toString());
        }
        List<CalendarInterval> mergedList = CalendarInterval.mergeIntersectingWithGapScaleIntervalsNotOrdered(croppedList, 1000);
        
        i = 1;
        System.out.println("Merged:");
        for (CalendarInterval calendarInterval: mergedList){
            System.out.println("interval "+ (i++) +" = "+calendarInterval.toString());
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.from != null && calendarInterval.getFrom() != null) {
            if (this.from.compareTo(calendarInterval.getFrom()) != 0) {
                return false;
            }
        } else if (this.from == null && calendarInterval.getFrom() != null) {
            return false;
        } else if (calendarInterval.getFrom() == null && this.from != null) {
            return false;
        }

        if (this.to != null && calendarInterval.getTo() != null) {
            if (this.to.compareTo(calendarInterval.getTo()) != 0) {
                return false;
            }
        } else if (this.to == null && calendarInterval.getTo() != null) {
            return false;
        } else if (calendarInterval.getTo() == null && this.to != null) {
            return false;
        }
        return true;
    }
