...
import ru.runa.bpm.job.executor.JobExecutorThread;

/**
 * Main class for RunaWFE caching. Register {@link ChangeListener} there to receive events on objects change 
 * and transaction complete. 
 * This class implements {@linkplain Synchronization} because in {@linkplain JobExecutorThread} to receive transaction complete events. 
 */
public class CachingLogic implements Synchronization {

    /**
     * Map from {@link Thread} to change listeners, which must be notified on thread transaction complete.
     * Then thread transaction change some objects, affected listeners stored there.  
     */
    private static Map<Thread, Set<ChangeListener>> dirtyThreads = new ConcurrentHashMap<Thread, Set<ChangeListener>>();

    /**
     * {@link ChangeListener}, which must be notified, when unrecognized object change.
     * All registered listeners contains in this collection.
     */
     ...
     * @param listener Listener, which must receive events.
     */
    ...
    /**
     * Notify registered listeners on unrecognized object change.
    ...
    /**
     * Notify registered listeners on executor related object change.
     * Only registered listeners, implementing {@link ExecutorChangeListener} will be notified.
    ...
    /**
     * Notify registered listeners on substitution related object change.
     * Only registered listeners, implementing {@link SubstitutionChangeListener} will be notified.
     * @param object Changed object.
     * @param currentState Current state of object properties.
     * @param previousState Previous state of object properties.
     * @param propertyNames Property names (same order as in currentState).
    ...
    /**
     * Notify registered listeners on task related object change.
     * Only registered listeners, implementing {@link TaskChangeListener} will be notified.
     * @param object Changed object.
     * @param currentState Current state of object properties.
     * @param previousState Previous state of object properties.
     * @param propertyNames Property names (same order as in currentState).
     ...
     * Only registered listeners, implementing {@link ProcessDefChangeListener} will be notified.
     * @param object Changed object.
     * @param currentState Current state of object properties.
     * @param previousState Previous state of object properties.
     * @param propertyNames Property names (same order as in currentState).
     ...
     * @param notifyThis Listeners to notify.
     * @param object Changed object.
     * @param currentState Current state of object properties.
     * @param previousState Previous state of object properties.
     * @param propertyNames Property names (same order as in currentState).
    ...
    /**
     * Called, then thread transaction is completed. If thread transaction change nothing, when do nothing.
     * If thread transaction change some objects, when all related listeners is notified on transaction complete.
     * All related listeners first receive markTransactionComplete event, after what
    ...
    /**
     * Return cache instance from cache control instance.
     * If control instance already initialized with cache instance, then returning it.
     * If control instance not initialized with cache instance, then cache instance is created and cache control initialized 
     * with created cache (if cache instance is not locked).
     * <p>This call can be blocked until change transactions will be complete.</p> 
     * @param <CacheImpl> Type of cache, controlled by cache control component.
     * @param cache Cache control component.
    ...
    /**
     * Try to get cache implementation from cache control. If no cache implementation in cache control or
     * thread transaction is change some objects, then return null. If cache control is already processing initiation,
     * then current thread blocking until initiation complete.
     * @param <CacheImpl> Type of cache, controlled by cache control component.
     * @param cache Cache control component.
        ...
        synchronized (CachingLogic.class) {
            while (true) {
                try {
                    ...
                    if (!cache.isLocked() && !cache.isInInitiate() && !isWriteTransaction()) { // Cache is steel not initiated and no initiate in progress - mark it as initiateInProgress and initiate. 
