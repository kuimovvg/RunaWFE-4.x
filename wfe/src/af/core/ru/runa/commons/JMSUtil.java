...
public class JMSUtil {
    ...
    public static ObjectMessage sendMessage(List<VariableMapping> data, Map<String, Object> variables) throws JMSException, NamingException {
        init();
        QueueSender sender = session.createSender(queue);
        HashMap<String, Object> map = new HashMap<String, Object>();
        for (VariableMapping variableMapping : data) {
            if (!variableMapping.isPropertySelector()) {
                map.put(variableMapping.getMappedName(), variables.get(variableMapping.getName()));
            }
        }
        ...
        for (VariableMapping variableMapping : data) {
            if (variableMapping.isPropertySelector()) {
                ...
                if (value.startsWith("${")) {
                    Object tmpValue = variables.get(value.substring(2, value.length() - 1));
                    value = (tmpValue != null) ? tmpValue.toString() : "";
                }
                message.setStringProperty(variableMapping.getName(), value);
            }
        }
    ...
    public static String toString(ObjectMessage message) throws JMSException {
        ...
        if (message.getJMSExpiration() != 0) {
            buffer.append("{JMSExpiration=").append(new Date(message.getJMSExpiration())).append("}<br>");
        }
