 ...
 */
package ru.runa.commons.sqltask;
...
import ru.runa.commons.VariablesUtil;
...
public class DatabaseTaskXmlParser {
     ...
     */
    public static DatabaseTask[] parse(byte[] bytes, Map<String, Object> processVariables) throws DatabaseTaskXmlParserException {
        try {
            InputStream is = new ByteArrayInputStream(bytes);
            Document document = XMLHelper.getDocument(is, PATH_ENTITY_RESOLVER);
            DatabaseTask[] databaseTasks = parseDatabaseTasks(document, processVariables);
    ...
    private static DatabaseTask[] parseDatabaseTasks(Document document, Map<String, Object> processVariables) {
        ...
        for (int i = 0; i < databaseTasks.length; i++) {
            ...
            String datasourceName = taskElement.getAttribute(DATASOURCE_ATTRIBUTE_NAME);
            AbstractQuery[] abstractQueries = parseTaskQueries(taskElement, processVariables);
            databaseTasks[i] = new DatabaseTask(datasourceName, abstractQueries);
        }
        return databaseTasks;
    }

    private static AbstractQuery[] parseTaskQueries(Element taskElement, Map<String, Object> processVariables) {
        ...
        for (int i = 0; i < queryElementList.getLength(); i++) {
            ...
            String sql = VariablesUtil.substitute(queryElement.getAttribute(SQL_ATTRIBUTE_NAME), processVariables);
