...
public class HibernateCompilerQueryBuilder {
     ...
     * @param batchPresentation {@link BatchPresentation}, used to build SQL query. 
     * @param parameters Parameters, used to build query.
     */
    public HibernateCompilerQueryBuilder(BatchPresentation batchPresentation, HibernateCompilerParameters parameters) {
    ...
    public Query build() {
        ...
        if (parameters.isCountQuery() || parameters.isOnlyIdentityLoad()) {
            return session.createSQLQuery(sqlRequest);
        } else {
            return session.createSQLQuery(sqlRequest).addEntity(batchPresentation.getClassPresentationClass());
        }
    }
    ...
    /**
     * Removes from SQL select clause all column names and 'as' statements.
     * So it's converts SQL like 'select TABLE.ID as T_ID, TABLE.NAME as T_N' to 'select TABLE.*'.
     * Actually only 'as' statements removing is required, but removing all columns is much simple and resulting request is much clever.
     * @param sqlRequest SQL request to tune select clause. 
     */
    private void tuneSelectClause(StringBuilder sqlRequest) {
        if (parameters.isCountQuery() || parameters.isOnlyIdentityLoad()) {
            return;
        }
        ...
        sqlRequest = sqlRequest.replace(posDot + 1, posFrom, "*");
    }
}
