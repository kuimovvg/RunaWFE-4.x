...
public class HibernateCompilerLeftJoinBuilder {
     ...
     * @param sqlRequest SQL query to inject left joins.
     */
    public void injectLeftJoin(StringBuilder sqlRequest) {
        ...
        for (FieldDescriptor field : batchPresentation.getAllFields()) {
            ...
            if (field.dbSources[0].getSourceObject().equals(batchPresentation.getClassPresentationClass())) {
                continue;
            }
     ...
     * @param sqlRequest SQL query to inject left joins.
     * @param field Field for replace join to left join.  
    ...
    /**
     * Get from query condition statement for removable field. E. q. it returns something
     * like this: (var.name = 'name'). 
     * @param sqlRequest SQL query to inject left joins.
     * @param joinedTableAlias SQL alias name, assigned to joining table. 
    ...
    /**
     * Get root join table alias from join restriction statement. Root join table is a 
     * table, we joining to (it must be placed right before 'left join' statement). 
     * @param joinedTableAlias SQL alias name, assigned to joining table. 
     * @param restriction Join restriction statement.
     ...
     * @param sqlRequest SQL request to remove joined table.
     * @param tableName Table name, which must be left joined.
     */
     ...
     * @param string String to parse identifier from.
     * @param idx Start index, to search identifier.
     * @param forwardSearch true, to search forward and false otherwise.
