...
public class SwimlaneManualElement extends SwimlaneGroupElement {
	...
	public void createGUI(Composite clientArea) {
        ...
        label.setText(Messages.getString("OrgFunction.Type"));
        	...
        	public void widgetSelected(SelectionEvent e) {
            	try {
                    ...
                    if (OrgFunctionDefinition.DEFAULT.getName().equals(displayName)) {
                        ...
                        currentDefinition = OrgFunctionsRegistry.getDefinitionByName(displayName);
                    }
                    if (classNameText != null) {
                      	String className = currentDefinition != null ? currentDefinition.getClassName() : "";
                       	classNameText.setText(className);
                    }
                    setOrgFunctionDefinitionName(currentDefinition.getKey());
                    updateSwimlane();
                    reloadParametersUI();
                } catch (Exception ex) {
	...
	public void open(String path, String swimlaneName, OrgFunctionDefinition currentDefinition) {
		...
		if (currentDefinition != null) {
			combo.setText(currentDefinition.getName());
            classNameText.setText(currentDefinition.getClassName());
            reloadParametersUI();
		} else {
			combo.select(-1);
		}
