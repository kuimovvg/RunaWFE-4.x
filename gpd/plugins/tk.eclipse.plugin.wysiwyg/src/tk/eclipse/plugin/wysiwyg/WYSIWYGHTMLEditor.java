...
import tk.eclipse.plugin.ftl.FreemarkerUtil;
...
public class WYSIWYGHTMLEditor extends MultiPageEditorPart implements StatusTextListener, IResourceChangeListener {
    ...
    protected void createPages() {
        ...
        try {
            ...
            DesignerLogger.logError(WYSIWYGPlugin.getResourceString("wysiwyg.design.create_error"), th);
        }
        try {
            addPage(sourceEditor, getEditorInput());
            setPageText(pageNumber++, WYSIWYGPlugin.getResourceString("wysiwyg.source.tab_name"));
        } catch (Exception ex) {
            DesignerLogger.logError(WYSIWYGPlugin.getResourceString("wysiwyg.source.create_error"), ex);
        }
    ...
    private String getDesignDocumentHTML(String prefix, String statusText) {
        ...
        if (isFtlFormat()) {
            try {
                ...
                DesignerLogger.logErrorWithoutDialog("freemarker html transform", e);
            }
        } else {
