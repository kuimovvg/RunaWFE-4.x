...
public class WYSIWYGPlugin extends AbstractUIPlugin {
    ...
    public File FCK_RESOURCES_FOLDER() {
        File result = useCKEditor3() ? new File(plugin.getStateLocation().toFile(), FCK_EDITOR()) : new File(new File(plugin.getStateLocation()
                .toFile(), FCK_EDITOR()), "editor");
        if (!result.exists()) {
            result.mkdirs();
        }
        return result;
    ...
    public static boolean useCKEditor3() {
        return PrefConstants.FORM_CK_EDITOR.equals(DesignerPlugin.getPrefString(PrefConstants.P_FORM_DEFAULT_FCK_EDITOR));
    }
}
