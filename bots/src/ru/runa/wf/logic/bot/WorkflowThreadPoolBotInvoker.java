...
public class WorkflowThreadPoolBotInvoker implements BotInvoker, Runnable {
    ...
    public void run() {
        ...
        for (WorkflowBot bot : botsTemplates) {
            try {
                Set<TaskStub> tasks = bot.getNewTasks();
                ...
                log.error("Bot execution failed.", e);
            } catch (AuthenticationException e) {
                configurationVersion = -1;
                log.error("Bot execution failed. Will recreate botstation settings and bots.", e);
            }
        }
    }
            ...
            if (botStation.getVersion() != configurationVersion) {
                ...
                for (Bot bot : bots) {
                    ...
                    try {
                        ...
                        log.error("Bot " + bot.getWfeUser() + " has incorrect password.");
                    }
                }
