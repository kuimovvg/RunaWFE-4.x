...
public class TextReportTaskHandler implements TaskHandler {
    ...
    public void handle(Subject subject, TaskStub taskStub) throws TaskHandlerException {
        try {
            ...
            byte[] fileContent = TextReportGenerator.getReportContent(settings, variables);
            ...
            executionService.completeTask(subject, taskStub.getId(), taskStub.getName(), taskStub.getTargetActor().getId(), vars);
            log.info("TextReportTaskHandler completed, task " + taskStub);
        } catch (Exception e) {
