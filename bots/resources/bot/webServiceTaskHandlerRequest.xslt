...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:script implements-prefix="runawfe" language="java" src="java:ru.runa.wf.logic.bot.WebServiceTaskHandler"/>
    ...
    <xsl:template match="runawfe:processInstanceGraph">
      <xsl:variable name="variableName" select="@processInstanceIdVariable"/>
      <xsl:value-of select="wfe_ve:getProcessInstanceGraph($variableName)"/>
    </xsl:template>


    <xsl:template match="runawfe:processId">
      <xsl:value-of select="wfe_ve:getProcessInstanceId()" />
    </xsl:template>
