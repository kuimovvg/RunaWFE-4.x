...
import ru.runa.gpd.htmleditor.template.HTMLTemplateManager;
import ru.runa.gpd.htmleditor.template.JavaScriptContextType;
import ru.runa.gpd.jseditor.launch.JavaScriptLibraryTable;
...
//				new AssistInfo(FUNCTION, "eval() - Object", "eval()"),
	...
	/**
	 * Returns Image from the assist proposal type.
	 * 
	...
	private static Image getImageFromType(int type){
		switch(type){
			case KEYWORD:
				return HTMLPlugin.getImage(HTMLPlugin.ICON_VALUE);
			case VARIABLE:
				return HTMLPlugin.getImage(HTMLPlugin.ICON_VARIABLE);
			case FUNCTION:
				return HTMLPlugin.getImage(HTMLPlugin.ICON_FUNCTION);
			case OBJECT:
				return HTMLPlugin.getImage(HTMLPlugin.ICON_CLASS);
			default:
				return null;
		}
	}
