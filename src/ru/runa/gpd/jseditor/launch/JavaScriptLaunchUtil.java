...
public class JavaScriptLaunchUtil {
	...
	public static File[] getClassPaths(){
		...
		File dir = EditorsPlugin.getDefault().getStateLocation().toFile();
		files = new File[]{
				new File(dir, "js.jar"),
	...
	public static void copyLibraries() throws CoreException {
		...
		for(int i=0;i<files.length;i++){
			if(!files[i].exists()){
				copyFile(EditorsPlugin.getDefault().getBundle().getEntry("/" + files[i].getName()),
						files[i]);
			}
		}
	}
	...
	private static void copyFile(URL url, File file) throws CoreException {
		try {
			...
			IStatus status = new Status(
					IStatus.ERROR, "EditorsPlugin", 0, ex.toString(), ex);
